...
class Animals: 
    ...
    def animal_capital(self, animal_price): #себестоимость животных по ценам контракта
        animals_capital = 0;
        for i in [animal_const.YOUNG,animal_const.ADULT,animal_const.OLD]:
            animals_capital += self.number[i]*animal_price[i]
        return animals_capital
    
    def change_animals_number(self, sold_animals_number): #изменение числа животных при продаже по условиям контракта
        for i in [animal_const.YOUNG,animal_const.ADULT,animal_const.OLD]:
            self.number[i] -= sold_animals_number[i]
            
    def change_animals_number_if_lack(self,sold_animals_number,forfeit): #изменение числа животных при продаже в случае невыполнения условий
        change_money = 0
        for i in [animal_const.YOUNG,animal_const.ADULT,animal_const.OLD]:
            self.number[i] = max(0,self.number[i]-self.contract.sold_animals_number[i])
            change_money += sold_animals_price[i]*min(self.number[i],sold_animals_number[i])
            change_money -= forfeit*(sold_animals_number[i]-min(self.number[i],sold_animals_number[i]))
        return change_money
...
class Contract:
    def __init__(self, remaining_period_r, feed_number_r, feed_price_r, sold_animals_number_r, sold_animals_price_r, forfeit_r):
    ...
    def animal_income(self, number_saled_animals): #прибыль от животных по контракту
        animals_income = 0
        for i in [animal_const.YOUNG,animal_const.ADULT,animal_const.OLD]:
            animals_income += number_saled_animals[i]*self.sold_animals_price[i]
        return animals_income
...
class Farm: 
    def __init__(self, contr, bank, numbers_of_animals, ava_feed):
        self.contract = contr
        ...
        self.total_capital += self.animals.animal_capital(self.contract.sold_animals_price)
    
    def animals_sale(self):
        if len([0 for i,j in zip(self.animals.number, self.contract.sold_animals_number) if i>=j]) == 3:
            self.animals.change_animals_number(self.contract.sold_animals_number)
            self.money_capital += self.contract.animal_income(self.contract.sold_animals_number)
        else:
            self.money_capital = self.animals.change_animals_number_if_lack(self.contract.sold_animals_number,self.contract.forfeit)
